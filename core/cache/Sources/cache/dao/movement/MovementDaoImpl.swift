 ...
 */
public class MovementDaoImpl : MovementDao {
    ...
    public func findMovement(byIdMovement idMovement: Int) -> Future<CachedMovement, Error> {
        Future { promise in
    ...
    ///
    /// - Returns: A Future returning an Array of CachedMovement or an Error
    public func findMovements(byIdParent idParent: Int) -> Future<[CachedMovement], Error> {
        Future { promise in
    ...
    public func findAllMovements() -> Future<[CachedMovement], Error> {
        Future { promise in
            guard let movements = Optional(self.realm.objects(CachedMovement.self)), !movements.isEmpty else {
                return promise(.failure(Realm.Error(Realm.Error.fail)))
            }
            promise(.success(movements.toArray()))
        }

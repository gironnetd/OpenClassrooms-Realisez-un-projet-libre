...
public class CachedAuthorDaoImpl : CachedAuthorDao {
    ...
    func findAuthor(byIdAuthor idAuthor: Int) -> Future<CachedAuthor, Error> {
        Future { promise in
            guard let author = try? Realm().objects(CachedAuthor.self)
    ...
    func findAuthors(byIdMovement idMovement: Int) -> Future<[CachedAuthor], Error> {
        Future { promise in
            guard let authors = try? Realm().objects(CachedMovement.self)
    ...
    func findAuthors(byIdTheme idTheme: Int) -> Future<[CachedAuthor], Error> {
        Future { promise in
            guard let authors = try? Realm().objects(CachedTheme.self)
    ...
    func findAllAuthors() -> Future<[CachedAuthor], Error> {
        Future { promise in
            guard let authors = try? Realm().objects(CachedAuthor.self), !authors.isEmpty else {
                return promise(.failure(Realm.Error.init(Realm.Error.fail)))
            }
            promise(.success(authors.toArray()))
        }

 ...
 */
public class ThemeDaoImpl : ThemeDao {
    ...
    public func findTheme(byIdTheme idTheme: Int) -> Future<CachedTheme, Error> {
        Future { promise in
    ...
    ///
    /// - Returns: A Future returning an Array of CachedTheme or an Error
    public func findThemes(byIdParent idParent: Int) -> Future<[CachedTheme], Error> {
        Future { promise in
    ...
    public func findAllThemes() -> Future<[CachedTheme], Error> {
        Future { promise in
            guard let themes = Optional(self.realm.objects(CachedTheme.self)), !themes.isEmpty else {
                return promise(.failure(Realm.Error(Realm.Error.fail)))
            }
            promise(.success(themes.toArray()))
        }

...
public class BookDaoImpl : BookDao {
    ...
    public func findBook(byIdBook idBook: Int) -> Future<CachedBook, Error> {
        Future { promise in
    ...
    ///
    /// - Returns: A Future returning an Array of CachedBook or an Error
    public func findBooks(byIdMovement idMovement: Int) -> Future<[CachedBook], Error> {
        Future { promise in
    ...
    ///
    /// - Returns: A Future returning an Array of CachedBook or an Error
    public func findBooks(byIdTheme idTheme: Int) -> Future<[CachedBook], Error> {
        Future { promise in
    ...
    ///
    /// - Returns: A Future returning a CachedBook or an Error
    public func findBook(byIdPresentation idPresentation: Int) -> Future<CachedBook, Error> {
        Future { promise in
            guard let book = self.realm.objects(CachedBook.self)
    ...
    public func findBook(byIdPicture idPicture: Int) -> Future<CachedBook, Error> {
        Future { promise in
    ...
    public func findAllBooks() -> Future<[CachedBook], Error> {
        Future { promise in
            guard let books = Optional(self.realm.objects(CachedBook.self)), !books.isEmpty else {
                return promise(.failure(Realm.Error.init(Realm.Error.fail)))
            }
            promise(.success(books.toArray()))
        }

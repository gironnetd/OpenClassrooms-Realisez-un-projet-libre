...
public class DefaultPictureRepository: PictureRepository {
    ...
    public init(pictureDao: PictureDao) {
    ...
    public func findPicture(byIdPicture idPicture: Int) -> AnyPublisher<Picture, Error> {
        self.pictureDao.findPicture(byIdPicture: idPicture).map { picture in picture.asExternalModel() }.eraseToAnyPublisher()
    }
    ...
    public func findAllPictures() -> AnyPublisher<[Picture], Error> {
        self.pictureDao.findAllPictures().map { pictures in
            pictures.map { picture in picture.asExternalModel() }
        }.eraseToAnyPublisher()
    }
}

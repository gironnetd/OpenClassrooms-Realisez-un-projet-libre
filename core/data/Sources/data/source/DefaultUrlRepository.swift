...
public class DefaultUrlRepository: UrlRepository {
    ...
    public func findUrls(byIdSource idSource: Int, sourceType: String) -> AnyPublisher<[Url], Error> {
        self.urlDao.findUrls(byIdSource: idSource, sourceType: sourceType).map { urls in
            urls.map { url in url.asExternalModel() }
        }.eraseToAnyPublisher()
    }
    ...
    public func findUrls(bySourceType sourceType: String) -> AnyPublisher<[Url], Error> {
        self.urlDao.findUrls(bySourceType: sourceType).map { urls in
            urls.map { url in url.asExternalModel() }
        }.eraseToAnyPublisher()
    }

...
public class DefaultAuthorRepository: AuthorRepository {
    ...
    public init(authorDao: AuthorDao) {
    ...
    public func findAuthor(byIdAuthor idAuthor: Int) -> AnyPublisher<Author, Error> {
        self.authorDao.findAuthor(byIdAuthor: idAuthor).map { author in author.asExternalModel() }.eraseToAnyPublisher()
    }
    ...
    public func findAllAuthors() -> AnyPublisher<[Author], Error> {
        self.authorDao.findAllAuthors().map { authors in
            authors.map { author in author.asExternalModel() }
        }.eraseToAnyPublisher()
    }
}

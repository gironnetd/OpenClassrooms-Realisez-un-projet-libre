...
class TestAuthorDao: AuthorDao {
    ...
    func findAuthors(byName name: String) -> Future<[CachedAuthor], Error> {
        Future { promise in
            guard let authors = Optional(self.authors.filter({ author in author.name == name })), !authors.isEmpty else {
                return promise(.failure(NSError(domain: "", code: 0, userInfo: nil)))
            }
    ...
    func findAuthors(byIdMovement idMovement: Int) -> Future<[CachedAuthor], Error> {
        Future { promise in
            guard let authors = Optional(self.authors.filter({ author in author.idMovement == idMovement })), !authors.isEmpty else {
                return promise(.failure(NSError(domain: "", code: 0, userInfo: nil)))
            }
    ...
    func findAuthors(byIdTheme idTheme: Int) -> Future<[CachedAuthor], Error> {
        Future { promise in
            guard let authors = Optional(self.themes.filter({ theme in theme.idTheme == idTheme}))?.first?.authors, !authors.isEmpty else {
                return promise(.failure(NSError(domain: "", code: 0, userInfo: nil)))
            }
    ...
    func findAllAuthors() -> Future<[CachedAuthor], Error> {
        Future { promise in
            guard !self.authors.isEmpty else {
                return promise(.failure(NSError(domain: "", code: 0, userInfo: nil)))
            }
